...
def main(args):
    ...
    if torch.cuda.is_available():
        ...
        Logger_log("no CUDA")
        from vzmi.mlx.common.runtime_information.OperatingSystem.InsideKubernetes.is_class import is_OperatingSystem_InsideKubernetes
        if is_OperatingSystem_InsideKubernetes():
            import socket, pdb, os; not socket.gethostname().endswith('ygrid.yahoo.com') and not os.environ.get('do_disable_PDB', '') and pdb.set_trace()
